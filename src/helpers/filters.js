export const filterByPrice = (products, { botPrice, topPrice }) => {
  return products.filter(
    product => product.price >= botPrice && product.price <= topPrice
  );
};
...
export const filterByColors = (products, colors) => {
  ...
  return products.filter(product =>
    colors.some(color => product.color.indexOf(color) !== -1)
  );
};
...
export const filterByTags = (products, tags) => {
  ...
  return products.filter(product =>
    tags.some(tag => product.tags.indexOf(tag) !== -1)
  );
};
...
export const filterBySizes = (products, sizes) => {
  ...
  return products.filter(product =>
    sizes.some(size => product.size.indexOf(size) !== -1)
  );
};
...
export const filter = (products, activeFilters) => {
  ...
  const filtered = filters.reduce(
    (res, current) => current.callback(res, activeFilters[current.key]),
    products
  );
...
export const sortByPriceLowToHigh = products => {
  return [].slice.call(products).sort((a, b) => a.price - b.price);
};

export const sortByPriceHighToLow = products => {
  return [].slice.call(products).sort((a, b) => b.price - a.price);
};

export const sortByRating = products => {};

export const sort = (products, type) => {
  const sorts = {
    'priceLowToHigh': sortByPriceLowToHigh,
    'priceHighToLow': sortByPriceHighToLow,
    'rating': sortByRating
  };

  return sorts[type](products);
};
